-- Insertar temas de fútbol en la categoría Fútbol
...
BEGIN
  ...
  IF futbol_category_id IS NULL THEN
    RAISE EXCEPTION 'La categoría Fútbol no existe. Ejecuta primero clean_and_rebuild.sql';
  END IF;
  ...
  RAISE NOTICE 'Temas de fútbol insertados correctamente';
END $$;

-- Verificar cuántos temas se insertaron
SELECT COUNT(*) as temas_futbol_insertados 
FROM topics 
WHERE category_id = (SELECT id FROM categories WHERE name = 'Fútbol');
